...
def calculate_env_durations(build_df):
    ...
    for env in ['dev', 'stg', 'prd']:
        if
    env not in env_durations_pivot.columns: \
...
def display_env_progression_chart(env_durations, selected_project):
    # Filter the durations data for the selected project
    project_data = env_durations[env_durations['projname'] == selected_project]
    ...
    # Create a line chart to show the progression
