...
with st.sidebar:
    ...
    with st.expander("🧭 Menu"):
        ...
        st.page_link("./pages/mock_data_dashboard.py", label="Mock Data Dashboard", icon="1️⃣", use_container_width=True)
        st.page_link("./pages/old.py", label="Old Dashboard", icon="1️⃣", use_container_width=True)

    st.sidebar.header("Filters")
    date_range = st.sidebar.date_input("#### Date Range", [], format="DD/MM/YYYY", min_value=builds_df['date_of_build'].min(), max_value=builds_df['date_of_build'].max())
    ...
    color_theme_list = ['magma', 'cividis', 'greens', 'inferno', 'blues', 'plasma', 'reds', 'rainbow', 'turbo', 'viridis']
...
with col[1]:
    ...
    st.altair_chart(team_usage_builds.bar_chart(filtered_builds_df, builds_per_team_filter, selected_color_theme), use_container_width=True)
    st.altair_chart(log_on_scatter.scatter(filtered_logins_df, periodic_filter, time_count_filter, selected_colors), use_container_width=True)
    st.altair_chart(number_of_builds_scatter.scatter(filtered_builds_df, periodic_filter, selected_colors), use_container_width=True)
